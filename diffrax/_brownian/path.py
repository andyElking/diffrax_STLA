...
class UnsafeBrownianPath(AbstractBrownianPath):
    ...
    def _evaluate_leaf(
        ...
        if levy_area is SpaceSpaceLevyArea:
            ...
            hh_01 = jr.normal(key_hh, shape.shape, shape.dtype) / math.sqrt(12)
            kk_01 = jr.normal(key_kk, shape.shape, shape.dtype) / math.sqrt(720)
            aa_01 = _foster_levy_area_dt1(key_aa, triu_indices, w_01, hh_01, kk_01)  # noqa
            aa = jnp.asarray(t1 - t0, dtype=shape.dtype) * aa_01
            w = w_01 * w_std
            hh = hh_01 * w_std
            kk = kk_01 * w_std
            levy_val = SpaceSpaceLevyArea(dt=dt, W=w, H=hh, K=kk, A=aa)

        elif levy_area is SpaceTimeTimeLevyArea:
...
def foster_levy_area(
    key, triu_indices, w: Array, hh: Array, kk: Array, dt: RealScalarLike = _one
):
    w_1 = w / jnp.sqrt(dt)
    hh_1 = hh / jnp.sqrt(dt)
    kk_1 = kk / jnp.sqrt(dt)

    levy_1 = _foster_levy_area_dt1(key, triu_indices, w_1, hh_1, kk_1)
    return dt * levy_1


def _foster_levy_area_dt1(key, triu_indices, w: Array, hh: Array, kk: Array):
