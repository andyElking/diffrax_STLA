...
def _directly_compute_coeffs(h, gamma, u):
    ...
    beta1 = jnp.exp(-al)
    a_half = (1 - beta_half) / gamma
    a1 = (1 - beta1) / gamma
    b_half = (beta_half + al / 2 - 1) / (gamma * al)
    b1 = (beta1 + al - 1) / (gamma * al)
    aa = a1 / h

    al2 = al**2
    al3 = al2 * al
    # chh_half = 3 * (beta_half * (8 + 4 * al) + al2 - 8) / (2 * al2 * gamma)
    # ckk_half = 30 * (beta_half * (2 * al * (al + 6) + 24) + al2 - 24) / (al3 * gamma)
    chh = 6 * (beta1 * (al + 2) + al - 2) / (al2 * gamma)
    ckk = 60 * (beta1 * (al * (al + 6) + 12) - al * (al - 6) - 12) / (al3 * gamma)
