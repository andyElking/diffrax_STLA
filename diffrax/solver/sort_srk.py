...
from ..custom_types import BoolScalarLike, DenseInfo, LevyVal, RealScalarLike, VF, Y
...
def _tay_cfs_single(c, u) -> _Coeffs:
    ...
    b_half = jnp.array(
        [0, 1 / 4, -c / 48, c2 / 384, -c3 / 3840, c4 / 46080], dtype=dtype
    )
...
class SORT(AbstractItoSolver):
    ...
    def step(
        ...
        assert levy.H is not None, "The diffusion should be a _LangevinDiffusionTerm"
        _, w = levy.W
        _, hh = levy.H
        _, kk = levy.K
        ...
        v_out = (
            cfs.beta1 * (v1 - uh / 6 * f0)
            - cfs.beta_half * 2 * uh / 3 * f1
            - uh / 6 * f_out
            + cfs.aa * rho_w_k
            - st.rho * (hh - 6 * kk)
        )
